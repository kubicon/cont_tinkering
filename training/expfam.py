...
Where `MixtureActorCritic` builds a multi-modal policy *additively in density
space* -- a categorical latent picking one of `num_components` Gaussians -- this
one builds it *additively in log-density space*:

    log p(a) = theta . phi(a) - log Z(theta)

with `phi` a **fixed** basis (RBF bumps across the action box, plus a few
monomials) and the network emitting only `theta`. Multi-modality is then a
property of one parameter vector rather than of a latent variable, so there is
no categorical head to carry, no component permutation to track, and no
mean/log-std pair to project or floor.

Why bother
----------
Entropic mirror ascent's exact proximal update is multiplicative on densities,
`p_{t+1} ~ p_t^a rho^b exp(eta Q)`, i.e. *additive on log-densities*. A Gaussian
family is not closed under that unless `Q` is quadratic, which is why the
Gaussian head in `idealized_mmd.py` falls back to a natural-gradient step while
the categorical head gets the closed-form one. A log-linear family is closed
under it (up to projecting `Q` onto the span of `phi`), so the whole policy
lives in the geometry the update actually moves in.

Exactness
---------
The policy is defined as the *piecewise-constant density on `grid_points`
bins*, not as a continuous density that is then discretized: `theta` induces
`pi = softmax(theta . phi(bin centers))`, a bin is drawn from `pi`, and the
action is uniform inside it. Consequently

  * `log p(a)` is exact -- it is the log-density of the distribution that was
    genuinely sampled from, so the PPO ratio is a ratio of two real densities;
  * the entropy is exact (`H(pi) + log bin_width`), not the sum of per-component
    entropies that `mixture_ppo_loss` uses in place of the intractable mixture
    entropy;
  * `KL(p || q)` is exact (the discrete KL of `pi_p` against `pi_q`), not the
    componentwise Gaussian upper bound.

Those last two are the terms the magnet and the trust region are made of, so
they are the point of the exercise rather than a side benefit.

Scope: one-shot `ZeroSumGame`s only (`train.py`'s `run_one_shot`). There are no
atoms and nothing illegal, so there is no `action_mask` and no categorical
factor here at all -- a hybrid poker-style version would put the atoms back in
front of a single "continuous" kind, but that is not what this module is for.

Multi-dimensional actions are handled by a **factored** family, one independent
1-D exponential family per action dimension (`log p(a) = sum_i log p_i(a_i)`),
which keeps the normalizer a `dim * grid_points` sum rather than a
`grid_points ** dim` one.
"""
