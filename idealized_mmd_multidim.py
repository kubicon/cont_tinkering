...
class Params(NamedTuple):
    ...
    means: jnp.ndarray    # (K, dim)     each component's per-axis Gaussian means
    log_std: jnp.ndarray  # (K, dim)     each component's per-axis log-std
...
def well_expectation(means, log_std, g: _Geom):
    """E_{a~N(mu_k, diag s_k)}[ sum_d D(a_d) ] per component -> (K,).
    ...
    """
    var = jnp.exp(2 * log_std)[..., None] + g.widths**2          # (K, dim, B)
...
def component_feat_moments(means, log_std, g: _Geom):
    """E_{a~N(mu,s)}[u(a)^j], j=1..max_order, elementwise over (K, dim) -> (K, dim, max_order)."""
    std = jnp.exp(log_std)
    shift = means - g.mid
    moments = [
...
def mixture_stats(p: Params, g: _Geom):
    ...
    well_comp = well_expectation(p.means, p.log_std, g)          # (K,)
    e_well = jnp.sum(w * well_comp)
    feat_centered = component_feat_moments(p.means, p.log_std, g) - g.target  # (K, dim, max_order)
    ...
    def step(p0, p1, m0, m1, floor, lam):
        def apply(p, opp, magnet, sign):
            ...
            def obj(pp):
                ...
                mag = mmd.magnet_coef * gaussian_kl(pp.means, pp.log_std, magnet.means, magnet.log_std)
            ...
            grad = jax.grad(obj)(p)
            means = p.means + (mmd.lr * jnp.exp(2 * p.log_std) * grad.means if mmd.train_means else 0.0)
            log_std = p.log_std + (mmd.lr * 0.5 * grad.log_std if mmd.train_std else 0.0)
    ...
    for t in range(mmd.steps):
        ...
        if t % log.every == 0 or t == mmd.steps - 1:
            history.append({
                ...
                "std0": np.asarray(jnp.exp(p0.log_std)).tolist(),
                "w1": [float(x) for x in jax.nn.softmax(p1.logits)],
                "means1": np.asarray(p1.means).tolist(),
                "std1": np.asarray(jnp.exp(p1.log_std)).tolist(),
            })
...
def build_init(game: MultiDimDecoyWellGame, mmd: MMDSection, init: InitSection) -> tuple[Params, Params]:
    ...
    log_std = jnp.full((k, dim), log_std_val, dtype=jnp.float64)
    log_std = jnp.clip(log_std, float(np.log(mmd.std_min)), float(np.log(mmd.std_max)))
    ...
    return (Params(logits=logits, means=means_for(m0), log_std=log_std),
            Params(logits=logits, means=means_for(m1), log_std=log_std))
